...
- **Hermes forwarding intake (multi-tenant):** `/api/intake` accepts `{job_url, telegram_id, chat_id}`, resolves user by `intake_secret_hash` (SHA-256, bound to users row), verifies body `telegram_id` matches the secret-bound user (403 on mismatch), stores `chat_id` on first contact, checks `check_active_run()` (409), enforces atomic daily cap (429), and runs scoped pipeline via `PipelineContext.from_user_id()` + `run_add()` in background with per-user error surfacing. Old `APPLYCLING_INTAKE_SECRET` env var replaced by per-user DB secrets.
