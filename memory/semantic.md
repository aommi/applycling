...
- **Hermes forwarding intake (multi-tenant):** `/api/intake` remains a secret-bound compatibility endpoint. `/api/forward` is the new localhost-only Hermes relay endpoint for all Telegram messages. It auto-creates users with `get_or_create_user_by_telegram()`, stores resume text for new users, preserves raw confirming corrections in `profile.pending_corrections`, activates approved users, builds `PipelineContext.from_user_id()` before charging quota, then schedules `_run_scoped_pipeline()`. Endpoint responses are bounded to relay fields only.
- **Web onboarding:** `/onboarding` and `/onboarding/confirm` provide a web-first resume paste and profile confirmation flow. Web onboarding creates a web-only `users` row with `telegram_id=NULL`; resume/profile are stored server-side in Postgres.
