...
- **Hosted dogfood deployment:** `docker-compose.prod.yml` targets a Kamatera VPS with Caddy TLS, Postgres, and bind-mounted `/opt/applycling/{data,output}` for config/profile data and durable artifacts. applycling/Postgres/Caddy run in Docker Compose under `/opt/applycling/app`; Hermes runs on the VPS host, not in Docker. Hosted UI auth uses `BasicAuthMiddleware`; `/healthz`, `/api/intake`, and localhost-only `/api/forward` are exempt.
...
- **Async workbench runs:** Submit/regenerate use `schedule_pipeline_run()` fire-and-forget. Intake (multi-tenant) uses `PipelineContext.from_user_id()` + `run_add()` via `_run_scoped_pipeline()` with per-user error surfacing via `notify_error_to_user()`.
...
- **Web onboarding (BasicAuth-gated):** `/onboarding` → `POST /onboarding/submit-resume` → `/onboarding/confirm` (`applycling/ui/routes.py`). Server-side HMAC-SHA256 tokens (`_sign_onboarding_user_id` / `_verify_onboarding_token`, constant-time compare) replace raw `user_id` in URLs and forms — closes IDOR. Empty, malformed, or tampered tokens 403 on both GET and POST. Token secret resolves `APPLYCLING_ONBOARDING_TOKEN_SECRET` → `APPLYCLING_UI_AUTH_PASSWORD` → `APPLYCLING_INTAKE_SECRET` (alpha fallback chain; GA cleanup tracked in `memory/working.md` Deferred). Web onboarding creates a web-only `users` row with `telegram_id=NULL`; resume/profile stored server-side in Postgres.
