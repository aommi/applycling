...
- **Telegram intake (Phase 2):** Inbound job URLs reach applycling via a dedicated Hermes profile (`~/.hermes/profiles/applycling/`) and wrapper command `applycling-hermes`. The Hermes gateway polls Telegram, extracts the URL, and forwards to `/api/intake`. Outbound delivery uses `applycling/telegram_notify.py`. Provision: `scripts/setup_hermes_telegram.sh`. See `DECISIONS.md` §2026-04-27.
- **Hosted dogfood deployment:** `docker-compose.prod.yml` targets a Kamatera VPS with Caddy TLS, Postgres, and bind-mounted `/opt/applycling/{data,output}` for config/profile data and durable artifacts. Hosted UI auth uses `BasicAuthMiddleware`; `/healthz` and `/api/intake` are exempt.
...
- **MCP server:** `applycling/mcp_server.py` exposes pipeline as MCP tools via FastMCP. `mcp serve` (stdio), `mcp config` (client JSON). Shipped tools: `add_job` (MCP-T1), `list_jobs(limit, status)` and `get_package(job_id)` (MCP-T2), `update_job_status(job_id, status, reason)` via `jobs_service.set_job_status()`, `interview_prep(job_id, stage, model, provider)` writes interview_prep.md, and `refine_package(job_id, feedback, artifacts, cascade, model, provider)` versions before overwriting (MCP-T3), `answer_questions(job_id, questions, model, provider)` drafts form answers, `critique_package(job_id, model, provider)` recruiter review, `generate_questions(job_id, stage, count, model, provider)` interview questions with STAR frameworks (MCP-T5). Shared helpers in `applycling/package_actions.py` — `ConfigurationError`, no CLI/UI deps. `list_jobs` reads via `get_store().load_jobs()` with Notion→SQLite fallback. `get_package` returns bounded markdown artifact content with 50K file / 200K total byte caps. Architecture contract at `architecture/MCP.md`. Non-author setup guide lives in root `README.md`; MCP-T4 local smoke checklist was recorded at `docs/planning/sprints/2026-05-mcp-alpha/qa/MCP-T4-local-trial.md` as a gitignored planning artifact.
...
- **MCP docs parity:** When MCP tools, parameters, setup, artifact behavior, timeouts, or client configuration change, update the relevant docs in the same PR: root `README.md` for non-author setup/use, `architecture/MCP.md` for durable contracts, and active sprint/planning docs when scope or acceptance changes.
